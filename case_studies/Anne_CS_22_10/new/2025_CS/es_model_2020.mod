...
# New objectives #
param lca_tech {TECHNOLOGIES} ;
param lca_res {RESOURCES} ;
...
param urban_land_constr {TECHNOLOGIES} >= 0; # Urban land use [km2*y]
param hh_constr {TECHNOLOGIES} >= 0; # Damage to human health [DALY] (daly = disability adjusted life years)
param ecosys_constr {TECHNOLOGIES} >= 0; # Damage to ecosystems [species*y]
param rsc_constr {TECHNOLOGIES} >= 0; # Damage to resources availability [b€]
param rcm_constr {TECHNOLOGIES} >= 0; # Raw critical material [kt/y]
param rcm_woCoal_constr {TECHNOLOGIES} >= 0; # Raw critical material [kt/y]
param agro_land_op {RESOURCES} >= 0; # Agricultural land use [km2*y]
param urban_land_op {RESOURCES} >= 0; # Urban land use [km2*y]
param hh_op {RESOURCES} >= 0; # Damage to human health [DALY] (daly = disability adjusted life years)
...
var Share_mobility_public >= 0 ;#share_mobility_public_min, <= share_mobility_public_max; # %_Public: Ratio [0; 1] public mobility over total passenger mobility
var Share_freight_train, >= 0 ;#share_freight_train_min, <= share_freight_train_max; # %_Fr,Rail: Ratio [0; 1] rail transport over total freight transport
...
var Share_freight_boat, >= 0 ;#share_freight_boat_min, <= share_freight_boat_max; # %_Fr,Boat: Ratio [0; 1] boat transport over total freight transport
...
var LCA_res {RESOURCES} >= -10000000; # LCA of the extraction/production/transport of the resources
var LCA_op {TECHNOLOGIES} >= -10000000; # LCA of the operation of the technologies
...
var TotalAGRO_LAND >= 0; # AGRO_LAND_tot [km2*year/year]: Total agricultural land occupation (AGRO_LAND) of the system
var AGRO_LAND_constr {TECHNOLOGIES} >= 0; #Total AGRO_LAND of the technologies
var AGRO_LAND_op {RESOURCES} >= 0; # Total yearly AGRO_LAND of the resources
var TotalURBAN_LAND >= 0; # URBAN_LAND_tot [km2*year/year]: Total urban land occupation (URBAN_LAND) of the system
var URBAN_LAND_constr {TECHNOLOGIES} >= 0; #Total URBAN_LAND of the technologies
var URBAN_LAND_op {RESOURCES} >= 0; # Total yearly URBAN_LAND of the resources
var TotalHH >= 0; # HH_tot [DALY/year]: Total human health impact (HH) of the system
var HH_constr {TECHNOLOGIES} >= 0; #Total HH of the technologies
var HH_op {RESOURCES} >= 0; # Total yearly HH of the resources
var TotalECOSYS >= 0; # ECOSYS_tot [species*year/year]: Total ecosystem impact (ECOSYS) of the system
var ECOSYS_constr {TECHNOLOGIES} >= 0; #Total ECOSYS of the technologies
var ECOSYS_op {RESOURCES} >= 0; # Total yearly ECOSYS of the resources
var TotalRSC >= 0; # RSC_tot [M$2013/year]: Total resource price increase (RSC) of the system
var RSC_constr {TECHNOLOGIES} >= 0; #Total RSC of the technologies
var RSC_op {RESOURCES} >= 0; # Total yearly RSC of the resources
...
subject to lca_op_calc {j in TECHNOLOGIES}:
	LCA_op [j] = lca_op [j] * sum {t in PERIODS, h in HOUR_OF_PERIOD [t], td in TYPICAL_DAY_OF_PERIOD [t]} (F_t [j, h, td] * t_op [h, td]);
...
# [Eq. 1001.3]
subject to crit_1_res_calc {i in RESOURCES}:
	Crit_1_res [i] = crit_1_res [i] * sum {t in PERIODS, h in HOUR_OF_PERIOD [t], td in TYPICAL_DAY_OF_PERIOD [t]} ( F_t [i, h, td] * t_op [h, td] );
...
# [Eq. 1002.2]
subject to crit_2_tech_calc {j in TECHNOLOGIES}:
	Crit_2_tech [j] = crit_2_tech [j] * F [j];

# [Eq. 1002.3]
subject to crit_2_res_calc {i in RESOURCES}:
	Crit_2_res [i] = crit_2_res [i] * sum {t in PERIODS, h in HOUR_OF_PERIOD [t], td in TYPICAL_DAY_OF_PERIOD [t]} ( F_t [i, h, td] * t_op [h, td] );

# [Eq. 1002.4]
subject to crit_2_op_calc {j in TECHNOLOGIES}:
	Crit_2_op [j] = crit_2_op [j] * sum {t in PERIODS, h in HOUR_OF_PERIOD [t], td in TYPICAL_DAY_OF_PERIOD [t]} (F_t [j, h, td] * t_op [h, td]);

# [Eq. 1002.1]
subject to totalCrit_2_calc:
	TotalCrit_2 = sum {j in TECHNOLOGIES} (Crit_2_tech [j] / lifetime [j]) + sum {i in RESOURCES} Crit_2_res [i] + sum {j in TECHNOLOGIES} Crit_2_op [j];

## Criteria 3
#-----------

# [Eq. 1003.2]
subject to crit_3_tech_calc {j in TECHNOLOGIES}:
	Crit_3_tech [j] = crit_3_tech [j] * F [j];

# [Eq. 1003.3]
subject to crit_3_res_calc {i in RESOURCES}:
	Crit_3_res [i] = crit_3_res [i] * sum {t in PERIODS, h in HOUR_OF_PERIOD [t], td in TYPICAL_DAY_OF_PERIOD [t]} ( F_t [i, h, td] * t_op [h, td] );

# [Eq. 1003.4]
subject to crit_3_op_calc {j in TECHNOLOGIES}:
	Crit_3_op [j] = crit_3_op [j] * sum {t in PERIODS, h in HOUR_OF_PERIOD [t], td in TYPICAL_DAY_OF_PERIOD [t]} (F_t [j, h, td] * t_op [h, td]);

# [Eq. 1003.1]
subject to totalCrit_3_calc:
	TotalCrit_3 = sum {j in TECHNOLOGIES} (Crit_3_tech [j] / lifetime [j]) + sum {i in RESOURCES} Crit_3_res [i] + sum {j in TECHNOLOGIES} Crit_3_op [j];
...
subject to elec_prod_NUCLEAR_2020 {j in TECHNOLOGIES}:
		sum {t in PERIODS, h in HOUR_OF_PERIOD [t], td in TYPICAL_DAY_OF_PERIOD [t]}  (F_t ['NUCLEAR', h , td] ) = 34400;
subject to elec_prod_CCGT_2020 {j in TECHNOLOGIES}:
		sum {t in PERIODS, h in HOUR_OF_PERIOD [t], td in TYPICAL_DAY_OF_PERIOD [t]} (F_t ['CCGT', h , td] ) = 26800;
subject to elec_prod_WIND_OFFSHORE_2020 {j in TECHNOLOGIES}:
		sum {t in PERIODS, h in HOUR_OF_PERIOD [t], td in TYPICAL_DAY_OF_PERIOD [t]} (F_t ['WIND_OFFSHORE', h , td] ) = 6989; #versus 6700 from ELIA, versus 6826 from EU (SPF_Economie gives 12.8 TWh for all the wind)
subject to elec_prod_WIND_ONSHORE_2020 {j in TECHNOLOGIES}:
		sum {t in PERIODS, h in HOUR_OF_PERIOD [t], td in TYPICAL_DAY_OF_PERIOD [t]}  (F_t ['WIND_ONSHORE' , h , td])  = 5811; #from SPF_Economie, versus 4100 from ELIA, versus 4052 from EU (SPF_Economie gives 12.8 TWh for all the wind)
subject to elec_prod_PV_2020 {j in TECHNOLOGIES}:
		sum {t in PERIODS, h in HOUR_OF_PERIOD [t], td in TYPICAL_DAY_OF_PERIOD [t]} (F_t ['PV', h , td] ) = 5100; #from SPF_Economie, versus 4300 from ELIA, versus 4327 from EU
subject to elec_prod_GEOTHERMAL_2020 {j in TECHNOLOGIES}:
		sum {t in PERIODS, h in HOUR_OF_PERIOD [t], td in TYPICAL_DAY_OF_PERIOD [t]} (F_t ['GEOTHERMAL', h , td] ) = 0; #from EU
subject to elec_prod_HYDRO_RIVER_2020 {j in TECHNOLOGIES}:
		sum {t in PERIODS, h in HOUR_OF_PERIOD [t], td in TYPICAL_DAY_OF_PERIOD [t]} (F_t ['HYDRO_RIVER', h , td] ) = 353; #from EU (confirmed by SPF_Economie, 300)
subject to elec_prod_COAL_US_2020 {j in TECHNOLOGIES}:
		sum {t in PERIODS, h in HOUR_OF_PERIOD [t], td in TYPICAL_DAY_OF_PERIOD [t]} (F_t ['COAL_US', h , td] ) = 1900; #from SPF_Economie
subject to elec_prod_COAL_IGCC_2020 {j in TECHNOLOGIES}:
		sum {t in PERIODS, h in HOUR_OF_PERIOD [t], td in TYPICAL_DAY_OF_PERIOD [t]} (F_t ['COAL_IGCC', h , td] ) = 0; #As done by GL for 2015
subject to no_elec_import_2020 {j in TECHNOLOGIES}:
		sum {t in PERIODS, h in HOUR_OF_PERIOD [t], td in TYPICAL_DAY_OF_PERIOD [t]} (F_t ['ELECTRICITY', h , td] ) = 0; #from SPF_Economie

### Production of bioethanol and biodiesel
subject to bioethanol_2020 {i in RESOURCES}:
		sum {t in PERIODS, h in HOUR_OF_PERIOD [t], td in TYPICAL_DAY_OF_PERIOD [t]} (F_t ['BIOETHANOL', h , td] ) = 1504.9;
subject to biodiesel_2020 {i in RESOURCES}:
		sum {t in PERIODS, h in HOUR_OF_PERIOD [t], td in TYPICAL_DAY_OF_PERIOD [t]} (F_t ['BIODIESEL', h , td] ) = 4139.1;

### No import of electrofuels in 2020
subject to no_H2_RE_2020 {i in RESOURCES}:
		sum {t in PERIODS, h in HOUR_OF_PERIOD [t], td in TYPICAL_DAY_OF_PERIOD [t]} (F_t ['H2_RE_IMPORT', h , td] ) = 0.0;
subject to no_GAS_RE_2020 {i in RESOURCES}:
		sum {t in PERIODS, h in HOUR_OF_PERIOD [t], td in TYPICAL_DAY_OF_PERIOD [t]} (F_t ['GAS_RE_IMPORT', h , td] ) = 0.0;
subject to no_AMMONIA_RE_2020 {i in RESOURCES}:
		sum {t in PERIODS, h in HOUR_OF_PERIOD [t], td in TYPICAL_DAY_OF_PERIOD [t]} (F_t ['AMMONIA_RE_IMPORT', h , td] ) = 0.0;
subject to no_METHANOL_RE_2020 {i in RESOURCES}:
		sum {t in PERIODS, h in HOUR_OF_PERIOD [t], td in TYPICAL_DAY_OF_PERIOD [t]} (F_t ['METHANOL_RE_IMPORT', h , td] ) = 0.0;

### Production of biomethane
subject to biogas_2020 {i in RESOURCES}:
		sum {t in PERIODS, h in HOUR_OF_PERIOD [t], td in TYPICAL_DAY_OF_PERIOD [t]} (F_t ['BIOMETHANATION', h , td] )  = 2500;

### Non-energy demand production ###
#subject to ned_prod_METHANOL_2020 {i in RESOURCES}:
		#sum {t in PERIODS, h in HOUR_OF_PERIOD [t], td in TYPICAL_DAY_OF_PERIOD [t]} (F_t ['METHANOL', h , td] ) = end_uses_input["NON_ENERGY"] * share_ned ["METHANOL"];
#subject to ned_prod_HVC_2020 {i in RESOURCES}:
		#sum {t in PERIODS, h in HOUR_OF_PERIOD [t], td in TYPICAL_DAY_OF_PERIOD [t]} (F_t ['OIL_TO_HVC', h , td] ) = end_uses_input["NON_ENERGY"] * share_ned ["HVC"];
#subject to ned_prod_AMMONIA_2020 {i in RESOURCES}:
		#sum {t in PERIODS, h in HOUR_OF_PERIOD [t], td in TYPICAL_DAY_OF_PERIOD [t]} (F_t ['HABER_BOSCH', h , td] ) = end_uses_input["NON_ENERGY"] * share_ned ["AMMONIA"];

fix Share_mobility_public := 0.188;
fix Share_freight_train := 0.101;
fix Share_freight_boat := 0.145;

##########################
### OBJECTIVE FUNCTION ###
##########################

## Multicriteria optimization (goal programming method)
