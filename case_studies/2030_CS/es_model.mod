...
subject to totalGWP_calc:
	...
	# TotalGWP = sum {i in RESOURCES} GWP_op [i]; #JUST RESOURCES
	TotalGWP = sum {j in TECHNOLOGIES} (GWP_constr [j] / lifetime [j]) + sum {i in RESOURCES} GWP_op [i]; #INCLUDING GREY EMISSIONS:
